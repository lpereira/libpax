...
static struct paxhashmap *pax_hashmap_new(void) {
  struct paxhashmap *phm =
      malloc(sizeof(struct paxhashmap) + 32 * sizeof(uint32_t));
  if (!phm) return NULL;
...
static struct paxhashmap *pax_hashmap_add(struct paxhashmap *phm,
  ...
  *is_new_entry = false;
  ...
  while (phm) {
    if (phm->elements[slot] == hash) return orig_phm;
    ...
    slot++;
    if (slot == orig_slot) phm = phm->next;
  }
